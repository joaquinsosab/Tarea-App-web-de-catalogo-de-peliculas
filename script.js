...
async function fetchPopularMovies() {
    try {
        ...
        const respuesta = await fetch(apiUrl + '/movie/popular?api_key=' + apiKey);
...
async function showMovieDetails(movieId) {
    try {
        const respuesta = await fetch(apiUrl + '/movie/' + movieId + '?api_key=' + apiKey);
        const movie = await respuesta.json();
        selectedMovieId = movie.id;
        detailsContainer.innerHTML = '<h3>' + movie.title + '</h3>' +
            '<img src="https://image.tmdb.org/t/p/w500' + movie.poster_path + '" alt="' + movie.title + '">' +
            '<p>' + movie.overview + '</p>' +
...
searchButton.addEventListener('click', async () => {
    ...
    if (query) {
        try {
            const respuesta = await fetch(apiUrl + '/search/movie?query=' + query + '&api_key=' + apiKey);
            const data = await respuesta.json();
            displayMovies(data.results);
        } catch (error) {
